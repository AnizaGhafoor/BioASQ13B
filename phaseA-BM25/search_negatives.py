...
def main():
    ...
    queries_path = "../../data/training/training13b_inflated_clean_wContents_IA.jsonl"
    queries, qrels_dict = get_queries( queries_path )   # get list of queries for each baseline


    with open(f"../results/13b/hard_negatives_IA_complete.jsonl", "w") as f:
        for baseline, query_list in queries.items():   
            ...
            neg_docs_wContent = add_content(baseline, negatives)
